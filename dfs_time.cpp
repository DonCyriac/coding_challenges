...
class Graph {
    ...
    void postorder(vector<bool>& discovered, int source, int &time) {
        if(discovered[source]) return;
        int start_time = time++;
        discovered[source] = true;
        
        for (auto child: adjList[source]) {
            postorder(discovered, child, time);
        }
        int end_time = time;
        cout << source << " "<<start_time<<" "<<end_time<<"\n";
        return;        
    }

     void preorder(vector<bool>& discovered, int source, int &time) {
        if(discovered[source]) return;
        ...
        for (auto child: adjList[source]) {
            preorder(discovered, child, time);
        }
